...
namespace Former.Models
{
    public class UpdateHandlers
    {
        ...
        private async Task OrderUpdateHandle(ChangesType changesType, Order order)
        {
            ...
            if (changesType == ChangesType.CHANGES_TYPE_PARTITIAL) await _historyClient.WriteOrder(order, ChangesType.CHANGES_TYPE_PARTITIAL, Converters.ConvertMetadata(_metadata), "Counter order initialized");
            if (changesType == ChangesType.CHANGES_TYPE_DELETE) await _historyClient.WriteOrder(order, ChangesType.CHANGES_TYPE_DELETE, Converters.ConvertMetadata( _metadata), "Counter order filled");
        }
        ...
        private async Task FitPrices()
        {
            ...
            foreach (var (key, order) in ordersSuitableForUpdate)
            {
                ...
                var ammendOrderResponse = await _tradeMarketClient.AmendOrder(order.Id, fairPrice, Converters.ConvertMetadata(_metadata));
